    ...
    <div className="min-h-screen flex flex-col lg:flex-row relative">
      ...
      <div className="relative flex w-full lg:w-1/2 bg-[var(--sidebar-bg)] text-white items-center justify-center px-6 sm:px-10 lg:p-12 min-h-[40vh] lg:min-h-screen">
        ...
        <div className="relative text-center max-w-lg flex flex-col items-center">
          ...
          <h1 className="text-lg sm:text-xl lg:text-2xl font-bold leading-snug mb-4 text-[var(--text-primary)] font-[Montserrat]"> Plateforme intelligente d’aide au contrôle de qualité du registre des contribuables</h1>
          <p className="text-xs sm:text-sm text-[var(--text-secondary)] font-[Montserrat]">Direction Générale des Impôts - République de Madagascar</p>
        </div>
        <div className="absolute bottom-4 lg:bottom-6 text-[10px] sm:text-xs text-[var(--text-secondary)] text-center w-full px-4 font-[Montserrat]">© 2026 DGI - Tous droits réservés</div>
      </div>
        ...
        <div className="w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border)] shadow-xl rounded-xl sm:rounded-2xl p-6 sm:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm mb-6">
            <label className="flex items-center gap-2 text-[var(--text-secondary)]">
              <input type="checkbox" className="font-[Montserrat]"/>Se souvenir de moi
            </label>
            <a href="#" className="text-[var(--primary)] hover:underline font-[Montserrat]">Mot de passe oublié ?</a>
          </div>
          <Button variant="primary" onClick={handleLogin} className="w-full py-3 text-base font-[Montserrat]">Se connecter</Button>
        </div>
      </div>
    </div>
  );
